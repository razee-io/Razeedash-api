...
RAZEE_QUERY='query($orgId: String! $uuid: String!) { subscription(orgId: $orgId, uuid: $uuid) { name, orgId, groups, satcluster: clusterId, channelName, channelUuid, version, versionUuid, owner { id name }, uuid, created, rolloutStatus { successCount, errorCount }, remoteResources { cluster { clusterId, name }, searchableData, lastModified }, groupObjs { uuid, name, clusterCount, clusters { clusterId, syncedIdentities { id, syncStatus } } } } }'
