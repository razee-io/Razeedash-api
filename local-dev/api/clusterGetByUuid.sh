...
RAZEE_QUERY='query($orgId: String! $clusterId: String!) { clusterByClusterId( orgId: $orgId clusterId: $clusterId) { orgId, clusterId, created, updated, metadata, groups { uuid name }, status, syncedIdentities { id syncStatus syncMessage }, lastOrgKey { uuid, name } } }'
