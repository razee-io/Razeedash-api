...
RAZEE_QUERY='query($orgId: String! $filter: String $limit: Int $skip: Int) { clusterSearch( orgId: $orgId filter: $filter limit: $limit skip: $skip) { orgId, clusterId, created, updated, groups { uuid name }, metadata, registration, status, syncedIdentities { id, syncStatus, syncMessage }, lastOrgKey { uuid, name } } }'
