...
class PubSubImpl {
  ...
  async resourceChangedFunc(resource, logger) {
    ...
    if (this.enabled) {
      ...
      try {
        logger.debug({ op, (({ data, searchableData, ...o }) => o)(resource), topic }, 'Publishing resource updates');  // Log without data / searchable data as they may contain user names in errors.
